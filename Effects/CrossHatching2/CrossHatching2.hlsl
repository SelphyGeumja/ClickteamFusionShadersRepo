...
// Norme de gradient pour égaliser l'épaisseur en espace p
static const float G_D = 1.41421356; // sqrt(2) pour t = x ± y

float hatchDiagA(float2 p) { return line1D(p.x - p.y, fLineWidth * G_D); }
float hatchDiagB(float2 p) { return line1D(p.x + p.y, fLineWidth * G_D); }
